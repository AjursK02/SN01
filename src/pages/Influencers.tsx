...
const allOutfits = featuredCollections
  ...
  // For demo purposes, let's pretend some are from influencers
  .map((outfit, index) => {
    if (index % 3 === 0) { // Every third outfit is an "influencer" outfit
      return {
        ...outfit,
        person: `@${outfit.person.toLowerCase().replace(' ', '_')}`
      };
    }
    return outfit;
  })
  .filter((outfit, index) => index % 3 === 0); // Keep only influencer outfits
    ...
    <Layout>
      ...
      <div className="bg-sanav/5 py-12">
        <div className="container mx-auto px-4 sm:px-6">
          <h1 className="text-3xl font-bold mb-4">Influencer Fashion</h1>
          <p className="text-muted-foreground max-w-2xl">
            Discover and shop trending outfits worn by your favorite social media influencers.
          </p>
        </div>
      </div>
        ...
        <div className="container mx-auto px-4 sm:px-6 py-4">
          <div className="flex flex-col md:flex-row justify-between gap-4">
            ...
            <form onSubmit={handleSearch} className="relative max-w-md w-full">
              <input
                type="text"
                placeholder="Search by influencer or outfit..."
